...
pub async fn login(
    ...
    // 3. Generate access token (JWT) – lightweight, no I/O
    ...
    // Note: In production, replace password::hash with a fast hash (e.g., SHA‑256) for tokens.
    ...
    if state
        .refresh_token_repo
        .create(user.id, &refresh_token, expires_in) // Assuming create now takes raw token (as fixed earlier)
        .await
        .is_err()
    {
        return (
    ...
    // 5. Set cookies (in‑memory, negligible)
