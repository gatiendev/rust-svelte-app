...
use tower_cookies::cookie::time; // `time` is re-exported from `cookie`
...
fn set_auth_cookies(
    ...
    refresh_cookie.set_max_age(Some(refresh_max_age));
...
pub async fn login(
    ...
    tracing::info!("[TIMING] DB lookup: {:?}", lookup_time);

    // 2. Password verification (now faster)
    ...
    tracing::info!("[TIMING] Password verify: {:?}", verify_time);
    ...
    tracing::info!("[TIMING] Access token generation: {:?}", token_time);

    // 4. Generate refresh token & store in DB (no extra hash here)
    ...
    if state
        .refresh_token_repo
        .create(user.id, &refresh_token, expires_in) // Pass raw token
        .await
        .is_err()
    {
        return (
    ...
    tracing::info!(
        "[TIMING] Refresh token generation + DB insert: {:?}",
        refresh_time
    );

    // 5. Set cookies
    let access_max_age = time::Duration::seconds(state.config.access_token_expiration);
    let refresh_max_age = time::Duration::seconds(state.config.refresh_token_expiration);
    set_auth_cookies(
        &cookies,
        &access_token,
        &refresh_token,
    ...
    tracing::info!("[TIMING] TOTAL login handler: {:?}", total_time);
...
pub async fn logout(cookies: Cookies, State(state): State<AppState>) -> impl IntoResponse {
    ...
    // Directly delete using the raw token – the repository hashes it with SHA-256.
    if state
        .refresh_token_repo
