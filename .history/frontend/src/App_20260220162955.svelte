...
>
	<div class="w-full max-w-md">
		...
		<div
			class="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-8 text-white"
		>
			<!-- Symbol Header -->
			...
			<div class="mb-6">
				...
				>${Number(priceData.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
			</div>
            ...
            {parseFloat(priceData.change) >= 0 ? 'text-green-400' : 'text-red-400'}"
					>{parseFloat(priceData.change) >= 0 ? '▲' : '▼'} {Math.abs(parseFloat(priceData.change)).toFixed(2)}%</div>
				</div>

				<div class="bg-white/5 rounded-xl p-4">
					<div class="text-sm text-white/60 mb-1">Volume</div>
					<div class="text-2xl font-semibold text-white">{Number(priceData.volume).toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
				</div>
			</div>
			...
			{#if prevPrice !== parseFloat(priceData.price) && prevPrice !== 0}
				<div
					class="mt-4 text-center text-sm text-white/40 animate-pulse"
				>{parseFloat(priceData.price) > prevPrice ? '▲ Up' : '▼ Down'} from previous</div>
			{/if}
		</div>

		<!-- Footer Note -->
		<p class="mt-4 text-center text-sm text-white/40">Real-time data via Binance WebSocket</p>
	</div>
</main>
