<script lang="ts">
    ...
    const baseClasses =
        "font-medium rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 inline-flex items-center justify-center gap-2";

    const variantClasses = {
        primary:
            "bg-primary-600 hover:bg-primary-700 text-white shadow-md hover:shadow-lg focus:ring-primary-500 dark:bg-primary-600 dark:hover:bg-primary-700",
        secondary:
...
<button
    ...
    class="{baseClasses} {variantClasses[variant]} {sizeClasses[
        size
    ]} {disabledClasses} {fullWidth ? 'w-full' : ''}"
