...
// It returns a pointer with an empty clients map and a broadcast channel ready for use.

// clients: a map that will track active WebSocket connections (the bool value is often used as a placeholder – presence in the map indicates the client is connected).

// broadcast: an unbuffered channel that will be used to send messages to all connected clients.
...
func main() {
	...
	// Start the main HTTP server in a goroutine (so it doesn't block)
	go func() {
	...
	// --- Everything below now runs as expected ---

	// Your Binance streaming setup
	...
	// Start streaming prices (this will now run)
